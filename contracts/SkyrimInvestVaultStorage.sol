    ...
    Ownable
{
    ...
    PeriodState public currentState;
    ...
    mapping(TokenType => uint256) public investmentPricePerPeriod;
    // Exchange ratio of invested Share and invested TRA per period.
    mapping(TokenType => uint256) public TRAPricePerPeriod;
    // Details about user's investment.
    mapping(TokenType => mapping(address => AccountShareSnapshot)) public accountInvestments;
    // Supply interest amount info
    ...
    uint256 public JTSupplyRatesPerPeriod;

    struct TotalInvestmentSnapshot {
        uint256 pendingInvestment;       // In a new investing round, total amount of pending investment.
        uint256 lockedInvestment;        // In the last investing round, total amount of locked investment.
        uint256 unlockedInvestment;      // In the last investing round, total amount of unlocked investment.
        uint256 totalShare;              // Total share amount that users have invested.
        uint256 totalInvestmentByUsers;  // Total amount that users have invested.
    }
    ...
    struct InvestmentConfig {
        uint256 readyTime;   // Ready time
        uint256 onGoingTime; // OnGoingTime time
    }
